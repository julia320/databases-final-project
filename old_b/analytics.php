	...
	<body class="gray-bg">
        ...
        <?php
            ...
            if(isset($_POST['timeToMine'])) {
                ...
                $foundNone = true;
                echo "<br>";
                $_SESSION['currStats'] = "Analyzing course assocations with desired support of ".$_POST['support']."% and desired confidence of ".$_POST['confidence']."%.";
                // echo "<ul>";
                $associations = [];
                $counter = 0;
                        ...
                        if($i!=$j) {
                            ...
                            if($assoc_arr[$i][$j]/$numEntries >= $support) {
                                ...
                                if($assoc_arr[$i][$j]/$assoc_arr[$i][$i] >= $confidence) {
                                    $foundNone = false;
                                    $associations[$counter] = array(array_search($i, $arr_map), array_search($j, $arr_map));
                                    $counter++;
                                }
                            }
                        }
                ...
                // echo $support;
                // echo $confidence;
                $_SESSION['associations'] = $associations;
                if($foundNone) {
            ...
            echo $_SESSION['currStats'];

            //search box
            echo "<div style=\"text-align: center;\">";
            echo "<form method=\"post\" action=\"analytics.php\"><input type=\"text\" name=\"search\" placeholder=\"Search for Course A by name...\"/><input type=\"submit\" value=\"Search\"/>";
            echo "</form></div>";
            echo "<br>";
            $search = $_POST['search'];
            ...
            if (is_null($search)) {
                echo "<table>";
                echo "<thead><tr><th>Course A</th><th> ⇒ </th><th>Course B</th></tr></thead>";
                for($i=0; $i<count($_SESSION['associations']); $i++) {
                    $getAssocC1 = "select courseno, dept, name from course where crn=".$_SESSION['associations'][$i][0];
                    $assocResC1 = mysqli_query($connection, $getAssocC1);
                    $getAssocC2 = "select courseno, dept, name from course where crn=".$_SESSION['associations'][$i][1];
                ...
                echo "<thead><tr><th>Course A</th><th> ⇒ </th><th>Course B</th></tr></thead>";
                for($i=0; $i<count($_SESSION['associations']); $i++) {
                    $getAssocC1 = "select courseno, dept, name from course where crn=".$_SESSION['associations'][$i][0]." and name like'%".$search."%'";
                    $assocResC1 = mysqli_query($connection, $getAssocC1);
                    $getAssocC2 = "select courseno, dept, name from course where crn=".$_SESSION['associations'][$i][1];
                    $assocResC2 = mysqli_query($connection, $getAssocC2);
                    if(mysqli_num_rows($assocResC1) > 0) {
                        echo "<tr>";
                        while($row = mysqli_fetch_assoc($assocResC1)) {
                            echo "<td>";
                            echo $row['dept']." ".$row['courseno'].": ".$row['name'];
